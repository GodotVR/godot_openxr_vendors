...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    source_xml_path = os.path.split(class_def.filepath)[1]
    if source_xml_path.startswith('addons--w4gd--'):
        source_xml_path = source_xml_path[14:]
    if source_xml_path.endswith('.xml'):
        source_xml_path = source_xml_path[:-4]
    source_xml_path = source_xml_path.replace("--", "/")
    if '.gd.' in source_xml_path:
        source_xml_path = source_xml_path.split('.gd.')[0] + '.gd'
    source_gitlab_url = f"https://gitlab.com/W4Games/w4gd/-/blob/{get_git_branch()}/{source_xml_path}"

    f.write(".. DO NOT EDIT THIS FILE!!!\n")
    f.write(".. Generated automatically from W4GD sources.\n")
    f.write(f".. GDScript source: {source_gitlab_url}.\n\n")
    ...
    for group_name in CLASS_GROUPS:
        if group_name in grouped_classes:
            ...
            for class_name in grouped_classes[group_name]:
                ...
                abbr_class_name = class_name
                if abbr_class_name.startswith('res://addons/w4gd/'):
                    abbr_class_name = abbr_class_name[18:]
                if ':' in abbr_class_name:
                    abbr_class_name = ': '.join(abbr_class_name.split(':'))

                f.write(f"    {abbr_class_name} <class_{sanitize_class_name(class_name)}>\n")
