...
@onready
var composition_layer_quad: OpenXRCompositionLayerQuad = $XROrigin3D/OpenXRCompositionLayerQuad
@onready
var composition_layer_cylinder: OpenXRCompositionLayerCylinder = $XROrigin3D/OpenXRCompositionLayerCylinder
@onready
var composition_layer_equirect: OpenXRCompositionLayerEquirect = $XROrigin3D/OpenXRCompositionLayerEquirect
...
@onready
var settings_composition_layer: OpenXRCompositionLayerCylinder = $XROrigin3D/SettingsCompositionLayer
@onready
var secure_content_composition_layer: OpenXRCompositionLayerCylinder = $XROrigin3D/SecureContentCompositionLayer
@onready
var disabled_supersampling_mesh: MeshInstance3D = $XROrigin3D/SettingsCompositionLayer/DisabledSupersampling/MeshInstance3D
@onready
var normal_supersampling_mesh: MeshInstance3D = $XROrigin3D/SettingsCompositionLayer/NormalSupersampling/MeshInstance3D
@onready
var quality_supersampling_mesh: MeshInstance3D = $XROrigin3D/SettingsCompositionLayer/QualitySupersampling/MeshInstance3D
@onready
var disabled_sharpening_mesh: MeshInstance3D = $XROrigin3D/SettingsCompositionLayer/DisabledSharpening/MeshInstance3D
@onready
var normal_sharpening_mesh: MeshInstance3D = $XROrigin3D/SettingsCompositionLayer/NormalSharpening/MeshInstance3D
@onready
var quality_sharpening_mesh: MeshInstance3D = $XROrigin3D/SettingsCompositionLayer/QualitySharpening/MeshInstance3D
@onready
var display_secure_content_mesh: MeshInstance3D = $XROrigin3D/SecureContentCompositionLayer/DisplaySecureContent/MeshInstance3D
@onready
var replace_secure_content_mesh: MeshInstance3D = $XROrigin3D/SecureContentCompositionLayer/ReplaceSecureContent/MeshInstance3D
@onready
var exclude_secure_content_mesh: MeshInstance3D = $XROrigin3D/SecureContentCompositionLayer/ExcludeSecureContent/MeshInstance3D
...
func _process(_delta: float) -> void:
	...
	ray_intersection_right = composition_layer_cylinder.intersects_ray(
		right_controller_ray_cast.global_transform.origin,
		-right_controller_ray_cast.global_transform.basis.y
	)
	ray_intersection_left = composition_layer_cylinder.intersects_ray(
		left_controller_ray_cast.global_transform.origin,
		-left_controller_ray_cast.global_transform.basis.y
	)
	...
	ray_intersection_left = composition_layer_equirect.intersects_ray(
		left_controller_ray_cast.global_transform.origin,
		-left_controller_ray_cast.global_transform.basis.y
	)
	equirect_panel.update_pointers(ray_intersection_left, ray_intersection_right)

	ray_intersection_right = composition_layer_quad.intersects_ray(
		right_controller_ray_cast.global_transform.origin,
		-right_controller_ray_cast.global_transform.basis.y
	)
	ray_intersection_left = composition_layer_quad.intersects_ray(
		left_controller_ray_cast.global_transform.origin,
		-left_controller_ray_cast.global_transform.basis.y
	)
...
func update(button_name: String) -> void:
	...
	match button_name:
		"DisabledSupersampling":
			settings_composition_layer.set(
				"XR_FB_composition_layer_settings/manual/supersampling_mode",
				OpenXRFbCompositionLayerSettingsExtensionWrapper.SUPERSAMPLING_MODE_DISABLED
			)
			disabled_supersampling_mesh.get_surface_override_material(0).albedo_color = Color.GREEN
		"NormalSupersampling":
			settings_composition_layer.set(
				"XR_FB_composition_layer_settings/manual/supersampling_mode",
				OpenXRFbCompositionLayerSettingsExtensionWrapper.SUPERSAMPLING_MODE_NORMAL
			)
			normal_supersampling_mesh.get_surface_override_material(0).albedo_color = Color.GREEN
		"QualitySupersampling":
			settings_composition_layer.set(
				"XR_FB_composition_layer_settings/manual/supersampling_mode",
				OpenXRFbCompositionLayerSettingsExtensionWrapper.SUPERSAMPLING_MODE_QUALITY
			)
			quality_supersampling_mesh.get_surface_override_material(0).albedo_color = Color.GREEN
		"DisabledSharpening":
			settings_composition_layer.set(
				"XR_FB_composition_layer_settings/manual/sharpening_mode",
