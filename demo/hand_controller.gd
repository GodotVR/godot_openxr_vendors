...
func _process(_delta: float) -> void:
	var hand_tracker_name = (
		"/user/hand_tracker/left" if tracker == "left_hand" else "/user/hand_tracker/right"
	)
	var hand_tracker_hand: OpenXRInterface.Hand = (
		OpenXRInterface.HAND_LEFT if tracker == "left_hand" else OpenXRInterface.HAND_RIGHT
	)
	...
	if hand_tracker and hand_tracker.has_tracking_data:
		...
		if (
			hand_tracker.hand_tracking_source == XRHandTracker.HAND_TRACKING_SOURCE_CONTROLLER
			and render_model.has_render_model_node()
		):
			render_model.show()
			interface.set_motion_range(
				hand_tracker_hand, OpenXRInterface.HAND_MOTION_RANGE_CONFORM_TO_CONTROLLER
			)
		...
		# and controller model is available, hide it.
			...
			render_model.hide()
			interface.set_motion_range(
				hand_tracker_hand, OpenXRInterface.HAND_MOTION_RANGE_UNOBSTRUCTED
			)
